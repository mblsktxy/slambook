...
int main(int argc, char** argv) {
    if (argc != 2) {
        ...
        // cout << "Usage: dense_mapping /home/zhh/dataset/test_data" << endl;
        return -1;
    }
...
bool readDatasetFiles(const string& path, vector<string>& color_image_files, 
    ...
    while (!fin.eof()) {
		// 数据格式：图像文件名tx, ty, tz, qx, qy, qz, qw，注意是TWC而非TCW
        string image;
        fin >> image;
        double data[7];
...
bool updateDepthFilter(const Vector2d& pt_ref, const Vector2d& pt_curr, 
    ...
    // 方程
    // d_ref * f_ref = R_RC * (d_cur * f_cur) + t_RC, which is the same as
...
void showEpipolarMatch(const Mat& ref, const Mat& curr, 
