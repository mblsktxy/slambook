...
public:
    ...
    EdgeSE3ProjectDirect(Eigen::Vector3d point, float fx, float fy, 
                         float cx, float cy, cv::Mat *image): 
        x_world_(point), fx_(fx), fy_(fy), cx_(cx), cy_(cy), image_(image) {}

    virtual void computeError() {
        const VertexSE3Expmap* v = static_cast<const VertexSE3Expmap*>(_vertices[0]);
        // Get space point P's coordinates in the second camera frame  
        Eigen::Vector3d x_local = v->estimate().map(x_world_); // q in book
        // coordinates in camera frame -> pixel coordinates
...
bool poseEstimationDirect(const vector<Measurement> &measurements,
    ...
    typedef g2o::BlockSolver<g2o::BlockSolverTraits<6,1>> blockSolver; // 求解的向量是6*1的
    unique_ptr<blockSolver::LinearSolverType> linearSolver = 
        g2o::make_unique<g2o::LinearSolverDense<blockSolver::PoseMatrixType>>();
